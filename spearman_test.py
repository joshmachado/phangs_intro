...
for i in range(len(res)):
    ...
    'SIGV_1_2', 'SIGV_1_3', 'SIGV_2_3', 'RAD_1_2', 'RAD_1_3', 'RAD_2_3'))
    for j in range(len(sources)):
        ...
        if os.path.isfile(fp) == True:
            ...
            for k in range(len(prop)):
                ...
                third = third[~np.isnan(third)]
                stats[k] = [scipy.stats.spearmanr(first, second)[0], scipy.stats.spearmanr(first, third)[0], scipy.stats.spearmanr(second, third)[0]]
        df.loc[j] = [str(sources[i]), stats[0][0], stats[0][1], stats[0][2], stats[1][0], stats[1][1], stats[1][2], stats[2][0], stats[2][1], stats[2][2]]
